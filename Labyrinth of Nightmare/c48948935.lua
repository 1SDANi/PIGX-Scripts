...
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g2=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_MZONE,0,nil,tp)
	if chk==0 then return aux.SelectUnselectGroup(g1,e,tp,2,2,aux.ChkfMMZ(1),0)
		and aux.SelectUnselectGroup(g2,e,tp,1,1,aux.ChkfMMZ(1),0) end
	...
	if not #mg1==2 then return end
	Duel.Release(mg1,REASON_COST)
end
...
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsFaceup() and tc:IsRelateToEffect(e) then
		...
		local g=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_GRAVE,0,1,1,nil)
