...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	e3:SetTarget(aux.TargetBoolFunction(Card.IsType,TYPE_TRAP+TYPE_SPELL))
	c:RegisterEffect(e3)
	--disable effect
	...
	e5:SetTarget(aux.TargetBoolFunction(Card.IsType,TYPE_TRAP+TYPE_SPELL))
	c:RegisterEffect(e5)
end
function s.disop(e,tp,eg,ep,ev,re,r,rp)
	local tl=Duel.GetChainInfo(ev,CHAININFO_TRIGGERING_LOCATION)
	if tl==LOCATION_SZONE and re:IsActiveType(TYPE_TRAP+TYPE_SPELL) and re:GetHandler():GetControler(1-tp) then
...
function s.filter(c)
	return c:IsReleasable() and c:IsAttribute(ATTRIBUTE_EARTH)
end
