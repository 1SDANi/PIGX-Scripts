...
function s.tgcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return #eg==1 and tc:IsMonster()
end
function s.filter(c,att)
	return not c:IsAttribute(att) and c:IsMonster() and c:IsAbleToGrave()
end
function s.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsRelateToEffect(e) and Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_DECK,0,1,nil,eg:GetFirst():GetAttribute()) end
