...
function s.ctfilter(c)
	return c:IsLocation(LOCATION_SZONE) and not c:IsPreviousLocation(LOCATION_SZONE) and c:IsSetCard(0x1034) and c:IsFaceup()
		and c:IsType(TYPE_UNION) and c:IsControler(tp)
end
...
function s.pltg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ct>0 and Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingMatchingCard(s.plfilter,tp,LOCATION_DECK+LOCATION_HAND,0,1,nil) end
end
function s.plop(e,tp,eg,ep,ev,re,r,rp)
	local ct=e:GetLabel()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local ft=Duel.GetLocationCount(tp,LOCATION_SZONE)
	if ct>tf then ct=ft end
	local g=Duel.SelectMatchingCard(tp,s.plfilter,tp,LOCATION_DECK+LOCATION_HAND,0,1,ct,nil)
