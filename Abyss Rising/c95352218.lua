...
function s.sumlimit(e,c,sump,sumtype,sumpos,targetp)
	local at=s.getlevel(Duel.GetMatchingGroup(Card.IsFaceup,targetp or sump,LOCATION_MZONE,0,nil))
	if at==0 then return false end
	return c:GetLevel()~=at
end
function s.getlevel(g)
	local aat=0
	local tc=g:GetFirst()
...
function s.adjustop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g1==0 then s[tp]=0
		...
		local att=s.getlevel(g1)
		if (att&att-1)~=0 then
			if s[tp]==0 or (s[tp]&att)==0 then
				Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(id,0))
				att=Duel.AnnounceLevel(tp,1,att)
			else att=s[tp] end
		end
