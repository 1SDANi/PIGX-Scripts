...
function s.cn(e)
	return e:GetHandler():GetCounter(COUNTER_XYZ)<=0
end
...
function s.cs(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local cg=Duel.SelectMatchingCard(tp,s.cfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,2,e:GetHandler())
	Duel.Remove(cg,POS_FACEUP,REASON_COST)
	e:SetLabel(#cg)
end
function s.op(e,tp,eg,ep,ev,re,r,rp)
	local count=e:GetLabel()
	if Duel.NegateAttack() and count>1 then
