...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(s.filter,tp,LOCATION_GRAVE,0,1,nil) and Duel.IsExistingMatchingCard(Card.IsAbleToDeck,tp,0,LOCATION_ONFIELD,1,nil) end
	local ct=1
	if Duel.IsExistingTarget(s.filter,tp,LOCATION_GRAVE,0,2,nil) and Duel.IsExistingMatchingCard(Card.IsAbleToDeck,tp,0,LOCATION_ONFIELD,2,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
		ct=2
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g1=Duel.SelectTarget(tp,s.filter,tp,LOCATION_GRAVE,0,ct,ct,nil)
	local g2=Duel.SelectTarget(tp,aux.TRUE,tp,0,LOCATION_ONFIELD,ct,ct,nil)
	g1:Merge(g2)
...
function s.fusionfilter(c,fc,sumtype,sp,sub,mg,sg)
	...
	return c:IsLevel(lv) and c:IsAttribute(ATTRIBUTE_EARTH)
end
