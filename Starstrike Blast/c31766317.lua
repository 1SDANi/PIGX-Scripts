...
function s.rfilter(c)
	return c:IsRace(RACE_FIEND) and c:IsAbleToRemove() and aux.SpElimFilter(c,true)
end
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return #eg==1 and dc~=e:GetHandler() and dc:IsFaceup() and dc:IsLocation(LOCATION_MZONE) 
		and dc:IsRace(RACE_FIEND) and dc:IsControler(tp) and dc:IsReason(REASON_BATTLE+REASON_EFFECT) and not dc:IsReason(REASON_REPLACE) 
...
function s.filter(c,e,tp)
	return c:IsRace(RACE_FIEND) and c:GetCode()~=id and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
