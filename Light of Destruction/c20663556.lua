...
function s.costfilter(c,ft,tp)
	return c:IsFaceup() and c:IsSetCard(0x12) and (ft>0 or (c:IsControler(tp) and c:GetSequence()<5))
end
...
function s.filter(c,e,tp)
	return c:IsSetCard(0x12) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:IsLevelBelow(4) and not c:IsCode(id)
end
