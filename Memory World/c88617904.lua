...
s.listed_names={89839552}
function s.cfilter(c,ft,tp)
	return c:IsCode(89839552) and (ft>0 or (c:GetSequence()<5 and c:IsControler(tp))) and (c:IsFaceup() or c:IsControler(tp))
end
...
function s.filter(c,e,tp)
	return c:IsRace(RACE_WARRIOR) and c:IsLevelBelow(6) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
