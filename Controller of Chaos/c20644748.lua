...
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFieldGroupCount(tp,LOCATION_ONFIELD,0)<=4
		and Duel.GetFieldGroupCount(tp,0,LOCATION_ONFIELD)<=4
end
...
function s.aclimit(e,re,tp)
	...
	if re:IsActiveType(TYPE_FIELD) then
		return not Duel.GetFieldCard(tp,LOCATION_SZONE,4) and Duel.GetFieldGroupCount(tp,LOCATION_ONFIELD,0)>3
...
function s.setlimit(e,c,tp)
	return c:IsType(TYPE_FIELD) and not Duel.GetFieldCard(tp,LOCATION_SZONE,4) and Duel.GetFieldGroupCount(tp,LOCATION_ONFIELD,0)>3
end
