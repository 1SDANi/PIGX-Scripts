--Varian's Number 1: Infection Buzz King
local s,id=GetID()
function s.initial_effect(c)
	...
	e1:SetCondition(s.rmcn)
	e1:SetTarget(s.rmtg)
	e1:SetOperation(s.rmop)
	c:RegisterEffect(e1)
	aux.AddEREquipLimit(c,s.eqcn,function(ec,_,tp) return ec:IsControler(1-tp) end,s.equipop,e1)
	--destroy and damage
...
function s.rmcn(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsSummonType(SUMMON_TYPE_FUSION) and s.eqcn(e,tp,eg,ep,ev,re,r,rp)
end
...
function s.rmop(e,tp,eg,ep,ev,re,r,rp)
	if not s.eqcn(e,tp,eg,ep,ev,re,r,rp) then return end
	local g=Duel.GetFieldGroup(tp,0,LOCATION_EXTRA)
	Duel.ConfirmCards(tp,g)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local tc=Duel.SelectMatchingCard(tp,s.filter,tp,0,LOCATION_EXTRA,1,1,nil):GetFirst()
	s.equipop(c,e,tp,tc)
end
