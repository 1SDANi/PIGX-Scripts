...
function s.desreptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
	return not c:IsReason(REASON_REPLACE) and Duel.IsExistingMatchingCard(s.repfilter,tp,0,LOCATION_MZONE,1,c) and c:IsAbleToRemove() and c:IsCanRemoveCounter(tp,COUNTER_XYZ,1,REASON_COST) end
	if Duel.SelectYesNo(tp,c,96) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESREPLACE)
		c:RemoveCounter(tp,COUNTER_XYZ,1,REASON_COST)
		local g=Duel.SelectMatchingCard(tp,s.repfilter,tp,0,LOCATION_MZONE,1,1,c)
		g:AddCard(c)
...
function s.chaosfilter(c,e,tp)
	return c:IsFaceup() and c:IsCanBeFusionMaterial() and c:IsRace(RACE_DRAGON) and c:IsLevel(8) and not c:IsType(TYPE_FUSION)
end
...
function s.chaosop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsCanBeFusionMaterial() and not tc:IsImmuneToEffect(e) then
		...
		if eq and #eq>0 then ct=ct+eq end
