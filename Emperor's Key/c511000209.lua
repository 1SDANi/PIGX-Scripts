...
function s.spfilter(c,e,tp,mc)
	...
	return c:IsType(TYPE_FUSION) and c:IsAttribute(ATTRIBUTE_DIVINE) and c:IsLevelAbove(mc:GetLevel()) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false)
end
function s.tgfilter(c,e,tp)
	return c:IsFaceup() and c:IsCanBeFusionMaterial() and c:IsAbleToGrave() and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c) and c:IsSetCard(0x7f)
end
