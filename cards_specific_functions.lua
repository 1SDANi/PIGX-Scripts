...
function Auxiliary.GeminiNormalCondition(effect)
	local c=effect:GetHandler()
	return not c:IsGeminiState()
end
function Auxiliary.GeminiEffectCondition(effect)
	local c=effect:GetHandler()
	return c:IsFaceup() and c:IsGeminiState()
end
function Auxiliary.EnableGeminiAttribute(c)
	local e0=Effect.CreateEffect(c)
	e0:SetType(EFFECT_TYPE_SINGLE)
	e0:SetProperty(EFFECT_FLAG_SINGLE_RANGE+EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_IGNORE_IMMUNE)
	e0:SetCode(EFFECT_ADD_TYPE)
	e0:SetRange(LOCATION_ALL)
	e0:SetCondition(aux.GeminiNormalCondition)
	e0:SetValue(TYPE_NORMAL)
	c:RegisterEffect(e0)
	local e1=e0:Clone()
	e1:SetCode(EFFECT_ADD_TYPE)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCondition(aux.GeminiEffectCondition)
	...
	e3:SetCode(EFFECT_GEMINI_SUMMONABLE)
	c:RegisterEffect(e3)
end
...
function Auxiliary.SpiritReturnCondition(e,tp,eg,ep,ev,re,r,rp)
	...
	if e:IsHasType(EFFECT_TYPE_TRIGGER_F) then
		return not c:IsHasEffect(EFFECT_SPIRIT_MAYNOT_RETURN) and c:IsAbleToHand()
...
function Auxiliary.NeosReturnFilter(c,e,tp,fusc,mg)
	return c:IsControler(tp) and c:IsLocation(LOCATION_DECK+LOCATION_EXTRA)
...
function Auxiliary.SecurityTarget(e,_c)
    return _c:GetColumnGroup():IsExists(function(c,tp)
											return c:IsControler(tp) and c:IsFaceup() and c:IsType(TYPE_MONSTER) and c:IsSetCard(0x15a)
