...
function Fusion.ConditionFilterMix(c,fc,sub,sub2,contact,sumtype,matcheck,tp,...)
	...
	if not c:IsCanBeFusionMaterial(fc,sumtype) then return false end
...
function Fusion.CheckMixGoal(tp,sg,fc,sub,sub2,contact,sumtype,chkf,...)
	...
	return sg:IsExists(Fusion.CheckMix,1,nil,sg,g,fc,sub,sub2,contact,sumtype,tp,...) and
		...
		and ((not Fusion.CheckAdditional or Fusion.CheckAdditional(tp,sg,fc)) or (not Fusion.CheckAdditional2 or Fusion.CheckAdditional2(tp,sg,fc)))
end
...
function Fusion.CheckMixRep(mg,sg,g,fc,sub,sub2,contact,sumtype,chkf,tp,fun1,minc,maxc,fun2,...)
	if fun2 then
...
function Fusion.CheckMixRepFilter(c,mg,sg,g,fc,sub,sub2,contact,sumtype,chkf,tp,fun1,minc,maxc,fun2,...)
	if fun2(c,fc,sub,sub2,mg,sg,tp,contact,sumtype) then
		g:AddCard(c)
		local sub=sub and fun2(c,fc,false,sub2,mg,sg,tp,contact,sumtype)
		local res=Fusion.CheckMixRep(mg,sg,g,fc,sub,sub2,contact,sumtype,chkf,tp,fun1,minc,maxc,...)
		g:RemoveCard(c)
		return res
	end
	return false
end
function Fusion.CheckMixRepGoal(tp,mg,sg,mustg,fc,sub,sub2,contact,sumtype,chkf,fun1,minc,maxc,...)
	if #sg<minc+#{...} or #sg>maxc+#{...} then return false end
	local g=Group.CreateGroup()
	return Fusion.CheckMixRep(mg,sg,g,fc,sub,sub2,contact,sumtype,chkf,tp,fun1,minc,maxc,...) and (chkf==PLAYER_NONE or Duel.GetLocationCountFromEx(tp,tp,sg,fc)>0)
		and ((not Fusion.CheckAdditional or Fusion.CheckAdditional(tp,sg,fc)) or (not Fusion.CheckAdditional2 or Fusion.CheckAdditional2(tp,sg,fc)))
end
...
function Fusion.CheckSelectMixRep(tp,mg,sg,mustg,g,fc,sub,sub2,contact,sumtype,chkf,fun1,minc,maxc,...)
	if Fusion.CheckAdditional and not Fusion.CheckAdditional(tp,g,fc) and Fusion.CheckAdditional2 and not Fusion.CheckAdditional2(tp,g,fc) then return false end
...
function Fusion.SelectMixRep(c,tp,mg,sg,mustg,fc,sub,sub2,contact,sumtype,chkf,fun1,minc,maxc,...)
	...
	sg:AddCard(c)
	local res=false
	if (Fusion.CheckAdditional and not Fusion.CheckAdditional(tp,sg,fc)) and (Fusion.CheckAdditional2 and not Fusion.CheckAdditional2(tp,sg,fc)) then
		res=false
	elseif Fusion.CheckMixRepGoal(tp,mg,sg,mustg,fc,sub,sub2,contact,sumtype,chkf,fun1,minc,maxc,...) then
		res=true
	else
		local g=Group.CreateGroup()
...
function Fusion.CheckSelectMixRepUnfix(tp,mg,sg,mustg,g,fc,sub,sub2,chkf,minc,maxc,...)
	...
	if Fusion.CheckAdditional2 and not Fusion.CheckAdditional2(tp,g,fc) then return false end
	if chkf==PLAYER_NONE or Duel.GetLocationCountFromEx(tp,tp,g,fc)>0 then
...
function Fusion.AddContactProc(c,group,op,sumcon,condition,sumtype,settype,setcode,opc,desc,cannotBeLizard,protected,location)
	if c:IsStatus(STATUS_COPYING_EFFECT) then return end
	local mt=c.__index
	local t={}
