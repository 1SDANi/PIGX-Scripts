...
function s.ft(c)
	...
	return sseq<5 and (c:IsType(TYPE_MONSTER) and (seq==(sseq-1) or seq==(sseq+1))) or (c:IsType(TYPE_SPELL+TYPE_TRAP) and (seq==(sseq-2) or seq==(sseq+2)))
end
function s.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and chkc:IsControler(1-tp) and s.ft(chkc) end
	if chk==0 then return Duel.IsExistingTarget(s.ft,tp,0,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,s.ft,tp,0,LOCATION_ONFIELD,1,1,nil)
