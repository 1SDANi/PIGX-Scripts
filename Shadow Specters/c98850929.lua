...
function s.cfilter(c)
	return c:IsFaceup() and c:IsSummonType(SUMMON_TYPE_FUSION+SUMMON_TYPE_RITUAL+SUMMON_TYPE_TRIBUTE+SUMMON_TYPE_FLIP)
end
...
function s.filter3(c,e)
	return c:IsSpell() and c:IsAbleToHand() and c:IsCanBeEffectTarget(e) and not c:IsCode(id)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	local g=Duel.GetMatchingGroup(s.cfilter,tp,LOCATION_MZONE,0,nil)
	local ct=g:GetClassCount(s.typecast)
	local g1=Duel.GetMatchingGroup(s.filter1,tp,LOCATION_GRAVE,0,nil,e)
