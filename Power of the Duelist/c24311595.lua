...
function s.filter(c,tp)
	return c:IsLocation(LOCATION_GRAVE) and c:IsReason(REASON_BATTLE+REASON_EFFECT) and c:IsControler(tp) and c:IsSetCard(0x16) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if e:GetHandler():IsRelateToEffect(e) and sg and #sg>0 then
