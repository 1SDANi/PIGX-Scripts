...
function s.spfilter(c,class,lv,e,tp)
	return c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,true,true) and (c:IsSetCard(0x41) and c:IsLevelBelow(lv-1))
end
...
function s.spfilter2(c,class,lv,e,tp,code)
	return c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,true,true) and (c:IsSetCard(0x41) and c:IsCode(code))
end
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.SpecialSummon(tc,0,tp,tp,true,true,POS_FACEUP) then
		...
		if Duel.IsExistingMatchingCard(s.spfilter2,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil,class,c:GetLevel(),e,tp,tc:GetCode()) and
			Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
			local g2=Duel.SelectMatchingCard(tp,s.spfilter2,tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil,class,lv,e,tp,tc:GetCode())
			local tc2=g:GetFirst()
			if tc2 and Duel.SpecialSummon(tc2,0,tp,tp,true,true,POS_FACEUP) then
				tc2:RegisterFlagEffect(tc2:GetOriginalCode(),RESET_EVENT+RESETS_STANDARD_EXC_GRAVE,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringid(tc:GetOriginalCode(),1))
