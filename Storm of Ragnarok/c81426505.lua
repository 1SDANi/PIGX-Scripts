...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local t={}
	local p=1
	if Duel.IsExistingMatchingCard(aux.TRUE,tp,0,LOCATION_MZONE,1,nil) then t[p]=aux.Stringid(id,0) p=p+1 end
	if Duel.IsExistingMatchingCard(s.filter2,tp,0,LOCATION_ONFIELD,1,nil) then t[p]=aux.Stringid(id,1) p=p+1 end
	if Duel.IsExistingMatchingCard(aux.TRUE,tp,0,LOCATION_HAND,1,nil) then t[p]=aux.Stringid(id,2) p=p+1 end
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(id,3))
	local sel=Duel.SelectOption(tp,table.unpack(t))+1
	local opt=t[sel]-aux.Stringid(id,0)
	local sg=nil
	if opt==0 then sg=Duel.GetMatchingGroup(aux.TRUE,tp,0,LOCATION_MZONE,nil)
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif opt==1 then
		...
		sg=Duel.GetMatchingGroup(aux.TRUE,tp,0,LOCATION_HAND,nil)
	end
