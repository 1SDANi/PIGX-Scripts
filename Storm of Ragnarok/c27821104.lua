...
function s.filter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0x3d) and Duel.IsExistingMatchingCard(s.tfilter,tp,LOCATION_DECK+LOCATION_HAND,0,1,nil,c:GetAttack(),c:GetCode(),e,tp)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local sg=Duel.SelectMatchingCard(tp,s.tfilter,tp,LOCATION_DECK+LOCATION_HAND,0,1,1,nil,tc:GetAttack(),tc:GetCode(),e,tp)
	if #sg>0 then
