...
s.listed_names={511310105}
s.inffilter=aux.FilterFaceupFunction(Card.IsCode,511310105)
function s.actcon(e,tp,eg,ep,ev,re,r,rp)
	return not Duel.IsExistingMatchingCard(s.inffilter,tp,LOCATION_ONFIELD,0,1,nil)
end
...
function s.infcon(e,tp,eg,ep,ev,re,r,rp)
	return  Duel.IsExistingMatchingCard(s.inffilter,tp,LOCATION_ONFIELD,0,1,nil)
end
...
function s.acinffilter(c,left,tp)
	...
	if left>right then left,right=right,left end
	return c:IsFaceup() and c:IsCode(511310105) and Duel.IsExistingMatchingCard(s.infacfilter,tp,LOCATION_SZONE,0,1,nil,left,right,tp)
end
