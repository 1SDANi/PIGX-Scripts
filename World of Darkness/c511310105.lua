...
function s.initial_effect(c)
	...
	local e3=e2:Clone()
	e3:SetCategory(CATEGORY_ACTIVATE)
...
s.listed_names={511310104}
s.zerofilter=aux.FilterFaceupFunction(Card.IsCode,511310104)
function s.actcon(e,tp,eg,ep,ev,re,r,rp)
	return not Duel.IsExistingMatchingCard(s.zerofilter,tp,LOCATION_ONFIELD,0,1,nil)
end
function s.actfilter(c,tp)
	return c:IsFacedown() and c:GetSequence()<5 and c:GetActivateEffect():IsActivatable(tp,true,true)
end
function s.acttg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(s.actfilter,tp,LOCATION_SZONE,0,1,nil,tp)
		and c:GetFlagEffect(id)==0 end
	c:RegisterFlagEffect(id,RESET_CHAIN,0,0)
	Duel.SetOperationInfo(0,CATEGORY_ACTIVATE,nil,1,tp,LOCATION_SZONE)
end
...
function s.actop(e,tp,eg,ep,ev,re,r,rp)
	...
	local ag=Duel.GetMatchingGroup(s.actfilter,tp,LOCATION_SZONE,0,nil,tp)
...
function s.infcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(s.zerofilter,tp,LOCATION_ONFIELD,0,1,nil)
end
function s.infacfilter(c,left,right,tp)
	local seq=c:GetSequence()
	return c:IsFacedown() and left<seq and seq<right and c:GetActivateEffect():IsActivatable(tp,true,true)
end
function s.inftg(e,tp,eg,ep,ev,re,rs,rp,chk)
	...
	if chk==0 then
		...
		return Duel.IsExistingMatchingCard(s.infacfilter,tp,LOCATION_SZONE,0,1,nil,left,right,tp)
	end
	c:RegisterFlagEffect(id,RESET_CHAIN,0,0)
	Duel.SetOperationInfo(0,CATEGORY_ACTIVATE,nil,1,tp,LOCATION_SZONE)
end
function s.acinffilter(c,left,tp)
	...
	if left>right then left,right=right,left end
	return c:IsFaceup() and c:IsCode(511310104) and Duel.IsExistingMatchingCard(s.infacfilter,tp,LOCATION_SZONE,0,1,nil,left,right,tp)
end
function s.infop(e,tp,eg,ep,ev,re,r,rp)
	...
	local right=ic:GetSequence()
	if left>right then left,right=right,left end
	local ag=Duel.GetMatchingGroup(s.infacfilter,tp,LOCATION_SZONE,0,nil,left,right,tp)
	if #ag>0 then
