...
--Chaos Number 43: Chaos Number 43: Soul Puppeteer Demon
local s,id=GetID()
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsPlayerCanSpecialSummonMonster(tp,id+1,0,TYPES_TOKEN,-2,-2,1,RACE_FIEND,ATTRIBUTE_DARK) end
...
function s.rdcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return ((a:IsType(TYPE_TOKEN) and a:IsControler(tp)) or (d and d:IsType(TYPE_TOKEN)) and a:IsControler(tp)) and tp==ep
end
...
function s.atktg(e,c)
	return c:IsType(TYPE_TOKEN) and e:GetHandler():GetCounter(COUNTER_XYZ)>0
end
function s.atkvl(e,c)
	return e:GetHandler():GetCounter(COUNTER_XYZ)-1
end
function s.indtg(e,c)
	return c:IsType(TYPE_TOKEN)
end
function s.indcn(e,re,r,rp)
	return (r&REASON_BATTLE)~=0
end
function s.chaosfilter(c,e,tp)
	return c:IsFaceup() and c:IsCanBeFusionMaterial() and c:IsCode(56051086)
end
