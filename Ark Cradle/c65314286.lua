...
function s.lpcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetBattledGroupCount()>0 and (Duel.GetLP(tp)~=4000 or Duel.GetLP(1-tp)~=4000)
end
function s.lpop(e,tp,eg,ep,ev,re,r,rp)
	local sel=Duel.SelectOption(tp,aux.Stringid(id,1),aux.Stringid(id,3))
	if sel==0 then
		Duel.SetLP(tp,4000)
	else
