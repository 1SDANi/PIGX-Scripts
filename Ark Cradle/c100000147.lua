...
function s.filter2(c,at,lv1)
	...
	return lv2>0 and lv2~=lv1 and c:IsFaceup() and c:IsAttribute(at)
end
function s.filter1(c,tp)
	local lv1=c:GetLevel()
	return lv1>0 and c:IsFaceup() and Duel.IsExistingTarget(s.filter2,tp,LOCATION_MZONE,0,1,c,c:GetAttribute(),lv1)
end
