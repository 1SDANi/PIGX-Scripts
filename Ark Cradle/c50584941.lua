...
function s.cfilter(c,tp)
	...
	return c:IsSetCard(0x1045) and c:IsType(TYPE_FUSION) and c:IsAbleToRemoveAsCost() and aux.SpElimFilter(c,true)
...
function s.filter(c,code)
	return c:IsFaceup() and c:IsSetCard(0x1045) and c:IsType(TYPE_FUSION) and (not c:IsCode(code) or not c:IsOriginalCode(code))
end
