...
function s.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return not c:IsReason(REASON_REPLACE) and c:IsCanRemoveCounter(tp,COUNTER_XYZ,1,REASON_COST) end
	if Duel.SelectEffectYesNo(tp,c,96) then
		c:RemoveCounter(tp,COUNTER_XYZ,c:GetCounter(COUNTER_XYZ),REASON_COST)
		local e1=Effect.CreateEffect(c)
