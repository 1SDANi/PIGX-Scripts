...
function s.costfilter(c,hz)
	return c:IsRace(RACE_PSYCHIC) and c:IsAbleToRemoveAsCost() and aux.SpElimFilter(c,true)  and (hz or (c:IsLocation(LOCATION_MZONE) and c:IsInMainZone()))
end
...
function s.damval(e,re,val,r,rp,rc)
	if (r&REASON_BATTLE)~=0 then return 0
	else return val end
end
