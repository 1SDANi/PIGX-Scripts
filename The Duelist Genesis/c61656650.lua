...
function s.efcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return g:GetFirst():IsControler(tp) and dg and #dg>0 and g:GetFirst():IsType(TYPE_MONSTER)
end
...
function s.eftg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(s.filter,tp,0,LOCATION_ONFIELD,1,nil,ev) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,s.filter,tp,0,LOCATION_ONFIELD,1,1,nil,ev)
end
