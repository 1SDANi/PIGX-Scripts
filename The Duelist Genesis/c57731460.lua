...
function s.filter(c,e,tp)
	return not c:IsCode(id) and c:IsLevelBelow(4) and c:IsSetCard(0x19) and c:IsCanBeSpecialSummoned(e,107,tp,false,false)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.SpecialSummon(tc,103,tp,tp,false,false,POS_FACEUP)>0 then
		tc:RegisterFlagEffect(tc:GetOriginalCode(),RESET_EVENT+RESETS_STANDARD_DISABLE,0,0)
		if Duel.IsExistingMatchingCard(s.hfilter,tp,LOCATION_GRAVE,0,1,nil)>0 then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
			local g=Duel.SelectMatchingCard(tp,s.hfilter,tp,LOCATION_GRAVE,0,1,1,nil)
			if #g>0 then
