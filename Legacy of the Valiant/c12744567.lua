...
--Chaos Number 101: Silent Honor DARK
local s,id=GetID()
...
function s.filter(c)
	return c:IsAbleToChangeControler() and c:IsType(TYPE_FUSION) and c:IsFaceup()
end
...
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=Duel.GetFirstTarget()
	if tc and tc:IsRelateToEffect(e) and tc:IsType(TYPE_MONSTER) and tc:IsControler(1-tp) and s.filter(tc) then
		s.equipop(c,e,tp,tc)
	end
end
function s.chaosfilter(c,e,tp)
	return c:IsFaceup() and c:IsCanBeFusionMaterial() and c:IsCode(48739166)
end
