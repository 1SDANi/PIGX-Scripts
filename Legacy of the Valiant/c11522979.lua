...
--Chaos Number 69: Heraldry Crest of Horror
local s,id=GetID()
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and c:IsRelateToEffect(e) and c:IsFaceup() and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		...
		c:RegisterEffect(e2)
		if not (tc:IsType(TYPE_TRAPMONSTER) and tc:IsType(TYPE_TOKEN)) then
			c:CopyEffect(tc:GetOriginalCodeRule(),RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_STANDBY+RESET_SELF_TURN,1)
		end
	end
end
function s.chaosfilter(c,e,tp)
	return c:IsFaceup() and c:IsCanBeFusionMaterial() and c:IsCode(2407234)
end
