...
function s.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_ATKDEFCHANGE+CATEGORY_COUNTER)
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	if Duel.NegateAttack() and Duel.IsExistingMatchingCard(s.filter,tp,0,LOCATION_MZONE,1,nil) then
		...
		if #g>0 then
			...
			if ct>0 and lv>0 then
				local e1=Effect.CreateEffect(c)
				e1:SetType(EFFECT_TYPE_SINGLE)
				e1:SetCode(EFFECT_UPDATE_ATTACK)
				e1:SetValue(lv*ct*100)
