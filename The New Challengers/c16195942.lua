...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and chkc:IsAttackAbove(1) end
	if chk==0 then return Duel.IsExistingTarget(Card.IsAttackAbove,tp,0,LOCATION_MZONE,1,nil,1) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,Card.IsAttackAbove,tp,0,LOCATION_MZONE,1,1,nil,1)
end
