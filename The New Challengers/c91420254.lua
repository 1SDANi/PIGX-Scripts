...
function s.dfilter(c,tp)
	return c:IsLocation(LOCATION_MZONE) and c:IsFaceup() and c:IsType(TYPE_SPIRIT) and c:IsReason(REASON_BATTLE) and c:IsControler(tp) and not c:IsReason(REASON_REPLACE)
end
...
function s.value(e,c)
	return c:IsLocation(LOCATION_MZONE) and c:IsFaceup() and c:IsType(TYPE_SPIRIT) and c:IsReason(REASON_BATTLE) and c:IsControler(e:GetHandlerPlayer()) and not c:IsReason(REASON_REPLACE)
end
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Destroy(e:GetHandler(),REASON_EFFECT+REASON_REPLACE)
end
function s.efilter(e,te)
	return te:GetHandler():GetControler()~=e:GetHandler():GetControler() and c:IsFaceup() and c:IsType(TYPE_SPIRIT) and not te:IsHasProperty(EFFECT_FLAG_CARD_TARGET)
end
