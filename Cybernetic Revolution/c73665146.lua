...
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsReleasable() and e:GetHandler():IsCanRemoveCounter(tp,COUNTER_SPELL,5,REASON_COST) end
	e:GetHandler():RemoveCounter(tp,COUNTER_SPELL,1,REASON_COST)
	Duel.Release(e:GetHandler(),REASON_COST)
end
function s.spfilter(c,e,tp)
	return (c:IsCode(72443568) or (c:IsSetCard(0x41) and c:IsLevelBelow(4))) and c:IsCanBeSpecialSummoned(e,0,tp,true,true)
end
...
function s.addc(e,tp,eg,ep,ev,re,r,rp)
	if ep~=tp then
		e:GetHandler():AddCounter(COUNTER_SPELL,#eg)
	end
end
