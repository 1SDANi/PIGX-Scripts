...
function s.spfilter(c,e,tp,ft,rg)
	...
	return lv>0 and c:IsRace(RACE_FIEND) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		and rg:CheckWithSumGreater(Card.GetLevel,lv,ft,99)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if rg:CheckWithSumGreater(Card.GetLevel,lv,ft,99) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		local rm=rg:SelectWithSumGreater(tp,Card.GetLevel,lv,ft,99)
