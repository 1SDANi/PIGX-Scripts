...
function s.initial_effect(c)
	...
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_GEMINI)
...
function s.filter(c)
	return (c:IsType(TYPE_NORMAL) or (c:IsType(TYPE_GEMINI) and c:IsLocation(LOCATION_DECK))) and c:IsDiscardable()
end
...
function s.fusionfilter(c,fc,sumtype,sp,sub,mg,sg)
	...
	return c:IsLevelAbove(1) and c:IsType(TYPE_NORMAL+TYPE_GEMINI) and (not rg or not sg or (st==tg and #sg>1) or (st<tg and rg:CheckWithSumEqual(Card.GetLevel,tg-st,1,99)))
end
