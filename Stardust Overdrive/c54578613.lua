...
function s.eqval(ec,c,tp)
	return ec:IsSetCard(0x29) and c:IsType(TYPE_UNION) and ec:IsControler(tp)
end
function s.filter(c)
...
function s.descost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.costfilter,tp,LOCATION_SZONE,0,1,nil) and Duel.IsExistingMatchingCard(Card.IsDiscardable,tp,LOCATION_HAND,0,1,e:GetHandler()) end
