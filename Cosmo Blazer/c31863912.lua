...
function s.filter1(c,sp,e,lv)
	return c:IsFaceup() and c:GetSummonPlayer()==sp and c:GetLevel()==lv and c:IsAbleToHand() and c:IsRelateToEffect(e)
end
function s.filter2(c,sp,e)
	return c:IsFaceup() and c:GetSummonPlayer()==sp and c:IsAbleToHand() and c:IsRelateToEffect(e)
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==0 or not e:GetHandler():IsRelateToEffect(e) then return end
	local dc=Duel.TossDice(tp,1)
	if dc<6 then
		local g=Duel.GetChainInfo(0,CHAININFO_TARGET_CARDS):Filter(s.filter1,nil,1-tp,e,dc)
	else
		local g=Duel.GetChainInfo(0,CHAININFO_TARGET_CARDS):Filter(s.filter2,nil,1-tp,e)
	end
	if #g>0 then
