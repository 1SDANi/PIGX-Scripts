...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsPlayerCanSpecialSummonMonster(tp,id+1,0,TYPES_TOKEN,2000,0,6,RACE_MACHINE,ATTRIBUTE_WIND) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	for i=1,ft do
		...
		if Duel.SpecialSummonStep(token,0,tp,tp,false,false,POS_FACEUP) then
			...
			e3:SetValue(1)
			token:SetReset(RESET_EVENT+RESETS_STANDARD)
