...
function s.initial_effect(c)
	--Fusion Material
	c:EnableReviveLimit()
	Fusion.AddProcMix(c,true,true,92421852,aux.FilterBoolFunctionEx(Card.IsRace,RACE_MACHINE))
...
function s.filter(c,e,tp)
	return c:IsCode(1412158) and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0 and
		c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,true,true)
end
...
function s.spft(c,e,tp)
	return c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:IsCode(92421852)
end
